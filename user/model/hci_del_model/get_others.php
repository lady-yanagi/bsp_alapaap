...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if (mysqli_num_rows($validate_hostname) < 1):
		...
		$sql = mysqli_query($conn,"SELECT * FROM tbl_forms_others where hostname = '$hci_del_search_txt' and form_type = '1' and uid = '$uid' and status = '1' ");
		$count = mysqli_num_rows($sql);

		// If hostname is Exist!
		if ($count > 0) {
			$sql_2 = mysqli_query($conn,"SELECT * FROM tbl_forms_others where hostname = '$hci_del_search_txt' and form_type = '1-1' and uid = '$uid'  and status = '1' ");
